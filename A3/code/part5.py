...
import obj_loader
from utils import (init_game, draw_game, update_game, get_camera_params)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        mask1 = cv2.morphologyEx(
            maskGreen, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
        mask1 = cv2.morphologyEx(
            mask1, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8))
        maskGreen = cv2.morphologyEx(
            mask1, cv2.MORPH_DILATE, np.ones((3, 3), np.uint8))
        conts_g, h_g = cv2.findContours(
            maskGreen.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        maskBlack = mask2
        mask1 = cv2.morphologyEx(
            maskBlack, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
        mask1 = cv2.morphologyEx(
            mask1, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8))
        maskBlack = cv2.morphologyEx(
            mask1, cv2.MORPH_DILATE, np.ones((3, 3), np.uint8))
        conts_b, h_b = cv2.findContours(
            maskBlack.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        maxw = 0
        maxh = 0
        ind = -1
        ...
        cv2.imshow("frame", FRAME)
        GAME_STATE = update_game(GAME_STATE, SIZE, np.average(
            box, axis=0)[1], np.average(box_g, axis=0)[1])
